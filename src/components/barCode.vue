		...
		<div>
			...
			<ul>
				...
				<li v-for="item in items" v-bind:key="item.id">
					<barcode v-bind:value="item.code">
						程序出了点小问题
					</barcode>
				</li>
			</ul>
...
// 					}, 2000);
			...
			onSubmit: function() {
				...
				if (year < 2029) {
					...
					this.yearArray.key = String.fromCharCode(97 + year - 2030)
				}

				var month = this.date.month;
				if (month > 9) {
					this.monthArray.key = String.fromCharCode(87 + month)
				} else {
				...
				if (day > 9) {
					this.dayArray.key = String.fromCharCode(87 + day)
				} else {
